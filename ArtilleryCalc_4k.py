...
class ScreenshotDrawApp:
    def __init__(self, root):
        ...
        self.tk_image = ImageTk.PhotoImage(self.screenshot.resize((2880, 1620), Image.LANCZOS))

        # 创建画布
        self.canvas = tk.Canvas(root, width=2880, height=1620)
        self.canvas.pack()
    ...
    def on_click(self, event):
        # 将点击位置从画布坐标转换为截图坐标
        x_screenshot = int(event.x * self.screenshot.width / 2880)
        y_screenshot = int(event.y * self.screenshot.height / 1620)
